...
### Obtain empirical cdf for each cluster and intensity component
get_center_cumu_intensity_array = function(spks_time_mlist, stim_onset_vec, 
                                      clusters_list, 
                                      v_vec,
                                      N_component=1,
                                      v0 = 0.15, v1 = 0.1,
...
{  
  ...
  center_cumu_intensity_array = array(dim=c(N_clus, N_component, length(t_vec)))
  center_cumu_density_array = array(dim=c(N_clus, N_component, length(t_vec)))
  center_Nspks_mat = matrix(nrow=N_clus, ncol=N_component)
  for (q in 1:N_clus) {
    cumu_intensity_q = rep(0, length(t_vec))
    cumu_density_q = rep(0, length(t_vec))
    F_hat_q = 0
    if(length(clusters_list[[q]])>0){
      ...
      if (N_spks_q>0) {
        ...
        counts_vec = hist(spks_time_q, breaks=breaks, plot=FALSE)$counts 
        cumu_density_q = cumsum(counts_vec) / N_spks_q
      } else{
        cumu_density_q = rep(0,length(t_vec))
      }
      ...
      cumu_intensity_q = cumu_density_q*F_hat_q
    }
    
    
    
    center_cumu_intensity_array[q,1,] = cumu_intensity_q
    center_cumu_density_array[q,1,] = cumu_density_q
    center_Nspks_mat[q,1] = F_hat_q
    
  }
  
  return(list(center_cumu_intensity_array=center_cumu_intensity_array,
