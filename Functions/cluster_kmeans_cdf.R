...
cluster_kmeans_cdf = function(spks_time_mlist, stim_onset_vec, reaction_time_vec, 
                              clusters_list, 
                              v_vec,
                              N_component=1,
                              v0 = 0.15, v1 = 0.1,
                              ...
                              ...)
{
  ...
  res = est_timeshift_cumu_density(spks_time_mlist = spks_time_mlist,
  ...
  emp_cdf_array = array(dim = c(N_node,N_trial,length(t_vec)))
  for (id_node in 1:N_node) {
    for (id_trial in 1:N_trial) {
      ...
      dN_mi = hist(spks_time_mi_vec, breaks=c(t_vec[1]-t_unit,t_vec)+(t_unit/2),
                   plot=FALSE)$counts
      emp_cdf_mi = cumsum(dN_mi)/(N_spks_mi+.Machine$double.eps)
      
      N_spks_mat[id_node, id_trial] = N_spks_mi
      emp_cdf_array[id_node, id_trial, ] = emp_cdf_mi
    }
  }
  ...
  for (id_trial in 1:N_trial) {
    ### Def of dist_1: N_spks_mi*(sum(density_zi^2)*t_unit - 2*sum(density_zi*emp_cdf_mi))
    dist_1_mat = matrix(nrow=N_node, ncol=N_clus)
    N_spks_vec = N_spks_mat[ , id_trial]
    emp_cdf_mat = emp_cdf_array[ , id_trial, ]
    inner_prod_mat = emp_cdf_mat %*% t(center_cumu_density_array[, 1, ])*t_unit
    center_density_L2norm_vec = rowSums(center_cumu_density_array[, 1, ]^2)*t_unit
  ...
  for (i in 1:N_node) {
    dist_vec = dist_mat[i, ]
    if (length(which.min(dist_vec))>1){
      membership[i] = sample(which.min(dist_vec),1)
    } else{
