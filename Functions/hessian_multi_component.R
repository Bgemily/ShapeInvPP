hessian_multi_component = function(fft_f_target, 
                                    fft_f_origin_mat,
                                    n0_vec)
{
  ...
  for (id_component in 1:N_component) {
    ...
    hessian_mat[id_component, id_component] = gd_order2
    for (id_component_2 in 1:N_component) {
      if (id_component_2 != id_component) {
        gd_order2 = 2 * sum( Re( (-1i*2*pi*(l_vec/N_timegrid) * 
                                   exp(1i*2*pi*(-n0_vec[[id_component]])*l_vec/N_timegrid) * 
                                   fft_f_origin_mat[id_component, ]) * 
                                   Conj(-1i*2*pi*(l_vec/N_timegrid) * 
                                          exp(1i*2*pi*(-n0_vec[[id_component_2]])*l_vec/N_timegrid) * 
                                          fft_f_origin_mat[id_component_2, ]) ) ) 
        hessian_mat[id_component, id_component_2] = gd_order2
      }
    }
