...
extract_measurement_v2 = function(folder_path, param_name=NULL, measurement=c("ARI_mean", "F_mean_sq_err", "v_mean_sq_err")){
  ...
  for (param_value in param_value_vec) {
    file_name_vec = list.files(path = file.path(folder_path, param_value), full.names = T, recursive = TRUE)
    
    # Use lapply for batch loading
    results_list = lapply(file_name_vec, function(file) {
      load(file)
      return(results)
    })
    # Combine the list of results into a single list
    results = do.call(c, results_list)
    
    # Extract specified measurement for all replicates in `results`. 
    if (FALSE){
