...
setup = 'Compare_methods_Nclus1_v3.1'
method = 'shape_inv_pp'
...
if (TRUE) {
  for (timeshift_trial_max in timeshift_trial_max_list){
    default_setting = paste0("timeshift_trial_max=",timeshift_trial_max,",", 
                             'N_spks_total=50,N_subj=10,N_clus=1,clus_sep=1.4,key_time_comp2=-0.2')
  ...
  if (TRUE) {
    default_setting = paste0('N_trial=2,N_spks_total=50,N_subj=10,N_clus=1,clus_sep=1.4,key_time_comp2=-0.2')
    for (id_N_split in 1:N_split) {
      if (save_res_details & (id_N_split == 1)) {
        save_center_pdf_array = TRUE
      } else {
...
if(TRUE){
  ...
  default_setting = 'N_trial=2,N_spks_total=50,N_subj=10,N_clus=1,clus_sep=1.4,key_time_comp2=-0.2'
  for (id_N_split in 1:N_split) {
    if (save_res_details & (id_N_split == 1)) {
      save_center_pdf_array = TRUE
    } else {
