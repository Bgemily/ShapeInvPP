...
setup = 'Model_misspecification/Use_timevarying_baseline_v3.8'
...
gp_sigma_list = list(0, 1, 3, 5, 10, 20, 30, 40)
    ...
    for (id_N_split in 1:N_split){
      ...
      for (id_N_trial in 1:length(N_trial_list)){
        ...
        results <- foreach(j = 1:N_replicate) %dopar% {
          SEED = sample(1:1e7,1)
          tryCatch(main_shapeinvpp_timevarying_baseline(SEED = SEED, 
                                   N_trial = N_trial,
